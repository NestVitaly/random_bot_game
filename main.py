...
def random_game(message):
    ...
    bot.send_message(message.chat.id, f'Твоя случайная игра: <u>{game[0]}</u>', parse_mode='html', reply_markup=kb_exit)
...
def choice_timing(message):
    bot.send_message(message.chat.id, f'{message.from_user.username}, выбери продолжительность игры! '
                                      ...
                                      f'\n<b>Долгая 🧑‍🦽</b> от 60 минут', parse_mode='html', reply_markup=kb_timing)
...
def time_game(message):
    ...
    if any(keyword in value for keyword in ["Быстрая 🏃", "Средняя 🚶", "Долгая 🧑‍🦽"]):
        bot.send_photo(message.chat.id, ph_choice, f'Введи количество игроков или можешь выбрать случайную игру', reply_markup=kb_random_timing)
        bot.register_next_step_handler(message, lambda msg: duration_players_input(msg, value))
    else:
        bot.send_message(message.chat.id, 'Что-то пошло не так. Попробуй ещё раз, либо можешь вернуться в начало', reply_markup=kb_exit)
        bot.register_next_step_handler(message, time_game)
...
def duration_players_input(message, value):
    if message.text == 'Случайная игра':
        ...
        if isinstance(game, list):
            bot.send_message(message.chat.id, f'Твоя игра: <u>{game[0]}</u>', parse_mode='html', reply_markup=kb_exit)
        else:
            bot.send_message(message.chat.id, 'Игру не удалось подобрать, Вам очень грустно.', reply_markup=kb_exit)
    else:
        try:
            ...
            if players > 0:
                ...
                if game is not None:
                    bot.send_message(message.chat.id, f'Вот твоя игра: <u>{game[0]}</u>', parse_mode='html', reply_markup=kb_exit)
                else:
                    bot.send_message(message.chat.id, 'Игру не удалось подобрать, Вам очень грустно.', reply_markup=kb_exit)
        except ValueError:
...
def get_members(message):
    ...
    if game_ch:
        bot.send_message(message.chat.id, f'Твоя игра: <u>{game_ch}</u>', parse_mode='html', reply_markup=kb_exit)
    else:
